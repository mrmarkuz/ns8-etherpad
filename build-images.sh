...
# Reuse existing nodebuilder-etherpad container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-etherpad; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-etherpad -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.8-alpine3.19 docker.io/etherpad/etherpad:2.4.2" \
    "${container}"
# Commit the image
